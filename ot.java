...
class ot{
	...
	public static boolean siirto(){
		...
		if(mVuoro) { System.out.print("Mustan"); } else { System.out.print("Valkoisen"); }
		System.out.println(" vuoro");
		System.out.print("Anna koordinaatit: ");
		String syote = koso.readLine();
		if(syote.length()==2) {  } else { return false; }
		int[] k = { syote.codePointAt(1) , syote.codePointAt(0) };
		if(k[1]<=72&&k[1]>=65||k[1]<=104&&k[1]>=97) {  } else { return false; }
		if(k[0]>=49&&k[0]<=56) {  } else { return false; }
		if(k[1]<80)
		{
			k[1] = k[1] - 65;
		}
		...
		k[0] -= 49;
		if(pelitaulu[k[0]][k[1]] == tyhja) {  } else { return false; }
		boolean a = true;
		char etsittava;
		char oma;
		if(mVuoro)
	...
	public static void alusta(){
		for(byte i=0;i<pelitaulu.length;i++)
		{
			...
			{
				if(i==3&&j==3) { pelitaulu[i][j] = musta; }
				else if(i==3&&j==4) { pelitaulu[i][j] = valkoinen; }
				else if(i==4&&j==3) { pelitaulu[i][j] = valkoinen; }
				else if(i==4&&j==4) { pelitaulu[i][j] = musta; }
				else { pelitaulu[i][j] = tyhja; }
			}
		}
	}
